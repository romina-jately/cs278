...
const MOCK_FLARES: Flare[] = [
  { id: "f1", title: "Tanning on Cowell", host: "Lily K.", hostGradient: 1, going: 4, ttl: "ends in 2h", cover: "linear-gradient(135deg,#E6C788,#C9A86B 60%,#9A7D3F)" },
  { id: "f2", title: "Trader Joe's run", host: "Nora T.", hostGradient: 2, going: 2, ttl: "ends in 45m", cover: "linear-gradient(135deg,#DCE7DD,#5C8A65)" },
  { id: "f3", title: "Coupa study sesh", host: "Ava R.", hostGradient: 3, going: 6, ttl: "ends in 3h", cover: "linear-gradient(135deg,#B8A6E8,#6B5BCB)" },
  { id: "f4", title: "Love Island watch", host: "Sam G.", hostGradient: 4, going: 3, ttl: "ends 11pm", cover: "linear-gradient(135deg,#E8B5B5,#C77B7B)" },
];

const MOCK_PINNED_EVENT: Event = {
  id: "evt-retreat",
  title: "Sisterhood retreat",
  eyebrow: "Friday May 9",
  date: "Friday May 9",
  time: "7:00 PM",
...
const MOCK_EVENTS: Event[] = [
  MOCK_PINNED_EVENT,
  {
    ...
    date: "Monday May 12", time: "8:00 PM", location: "House library",
    cover: "alumni", tag: { label: "Mandatory", tone: "danger" },
    attendees: ["A","B","C","D"], going: 42, capacity: 60, reactions: 4, comments: 2,
  },
  {
    id: "evt-bake", title: "Bake sale", eyebrow: "Wednesday May 14",
    date: "Wednesday May 14", time: "11:00 AM – 3:00 PM", location: "The Quad",
    cover: "philanthropy", tag: { label: "Philanthropy", tone: "gold" },
    attendees: ["A","B","C","D"], going: 18, capacity: 60, reactions: 22, comments: 14,
  },
];

const MOCK_UPCOMING: UpcomingItem[] = [
  { id: "u1", day: "Mon", dayNum: "12", title: "Chapter meeting", sub: "House library · 8pm", tint: "lavender" },
  { id: "u2", day: "Wed", dayNum: "14", title: "Bake sale (philanthropy)", sub: "The Quad · 11–3", tint: "gold" },
  { id: "u3", day: "Thu", dayNum: "15", title: "Alumni mixer", sub: "Greek row · 6pm", tint: "dusty" },
];
...
const MOCK_MEMORIES: Memory[] = [
  { id: "m1", title: "Big-little reveal", cover: "philanthropy", count: 32 },
  { id: "m2", title: "Spring formal", cover: "formal", count: 84 },
  { id: "m3", title: "Beach day", cover: "beach", count: 56 },
  { id: "m4", title: "Date party", cover: "mixer", count: 48 },
];

const MOCK_EVENT_TASKS: Task[] = [
...
const MOCK_EVENT_GOING: Member[] = [
  { id: "g1", name: "Maya P.", gradient: 0, role: "Host · bringing snacks" },
  { id: "g2", name: "Lily K.", gradient: 1, role: "Driving · 4 seats" },
  { id: "g3", name: "Nora T.", gradient: 2, role: "Joining late" },
];
...
const MOCK_CHAPTER_THREAD: Message[] = [
...
const MOCK_FEATURED_ALUMNA: Member = {
  id: "rachel", name: "Rachel Diaz", gradient: 3, year: "'19", role: "Sr. PMM at Figma", location: "SF",
};

const MOCK_JOBS: Job[] = [
  { id: "j1", company: "Figma", role: "PMM intern", location: "SF", via: "Rachel D, '19", viaGradient: 3 },
  { id: "j2", company: "Stripe", role: "SWE intern", location: "Remote", via: "Iris W, '20", viaGradient: 5 },
  { id: "j3", company: "Goldman", role: "Summer analyst", location: "NYC", via: "Tess M, '18", viaGradient: 4 },
];

const MOCK_ALUMNAE: Member[] = [
  { id: "a1", name: "Rachel Diaz", year: "'19", role: "Sr. PMM, Figma", location: "SF", gradient: 3, hiring: true },
  { id: "a2", name: "Iris Wong", year: "'20", role: "SWE, Stripe", location: "Remote", gradient: 5, hiring: true },
  { id: "a3", name: "Tess Morgan", year: "'18", role: "Analyst, Goldman", location: "NYC", gradient: 4, hiring: true },
  { id: "a4", name: "Mia Levin", year: "'17", role: "Resident, Stanford Med", location: "Palo Alto", gradient: 2 },
  { id: "a5", name: "Jules Hart", year: "'16", role: "Founder, Patchwork", location: "LA", gradient: 0 },
];
...
export const api = {
  ...
  getFlares: (): Promise<Flare[]> =>
    USE_MOCKS ? delay(MOCK_FLARES) : request("/flares"),

  getPinnedEvent: (): Promise<Event> =>
    USE_MOCKS ? delay(MOCK_PINNED_EVENT) : request("/events/pinned"),

  getEvents: (): Promise<Event[]> =>
    USE_MOCKS ? delay(MOCK_EVENTS) : request("/events"),

  getEvent: (id: string): Promise<Event> =>
    USE_MOCKS
      ? delay(MOCK_EVENTS.find(e => e.id === id) ?? MOCK_PINNED_EVENT)
      : request(`/events/${id}`),
  ...
  getLeaderboard: (): Promise<LeaderboardEntry[]> =>
    USE_MOCKS ? delay(MOCK_LEADERBOARD) : request("/leaderboard"),

  getMemories: (): Promise<Memory[]> =>
    USE_MOCKS ? delay(MOCK_MEMORIES) : request("/memories"),

  getEventTasks: (_eventId: string): Promise<Task[]> =>
    USE_MOCKS ? delay(MOCK_EVENT_TASKS) : request(`/events/${_eventId}/tasks`),

  getEventGoing: (_eventId: string): Promise<Member[]> =>
    USE_MOCKS ? delay(MOCK_EVENT_GOING) : request(`/events/${_eventId}/going`),
  ...
  getThreadMessages: (_threadId: string): Promise<Message[]> =>
    USE_MOCKS ? delay(MOCK_CHAPTER_THREAD) : request(`/threads/${_threadId}/messages`),
  ...
  getFeaturedAlumna: (): Promise<Member> =>
    USE_MOCKS ? delay(MOCK_FEATURED_ALUMNA) : request("/alumnae/featured"),

  getJobs: (): Promise<Job[]> =>
    USE_MOCKS ? delay(MOCK_JOBS) : request("/jobs"),

  getAlumnae: (): Promise<Member[]> =>
    USE_MOCKS ? delay(MOCK_ALUMNAE) : request("/alumnae"),
